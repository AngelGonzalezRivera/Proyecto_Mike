...
return  db.get().query("Insert into jefe_de_grupo values(?,?,?,?)",[null,jefe_de_grupo.id_grupo,jefe_de_grupo.nombre,jefe_de_grupo.id_user],callback);
},
...
updatejefe_de_grupo:function(id,jefe_de_grupo,callback){
 return  db.get().query("update jefe_de_grupo set id_grupo=?,nombre=?,id_user=? where id=?",[jefe_de_grupo.id_grupo,jefe_de_grupo.nombre,jefe_de_grupo.id_user,jefe_de_grupo.status,id],callback);
}
};
