...
return  db.get().query("Insert into maestro values(?,?,?,?,?)",[null,maestro.nombre,maestro.apellido,maestro.status,maestro.id_user],callback);
},
...
updatemaestro:function(id,maestro,callback){
 return  db.get().query("update maestro set nombre=?,apellido=?,status=?,id_user=? where id=?",[maestro.nombre,maestro.apellido,maestro.status,maestro.id_user,id],callback);
},
