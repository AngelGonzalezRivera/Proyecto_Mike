...
return  db.get().query("Insert into maestro values(?,?,?,?)",[null,maestro.nombre,maestro.apellido,maestro.status],callback);
},
...
updatemaestro:function(id,maestro,callback){
 return  db.get().query("update maestro set nombre=?,apellido=?,status=? where id=?",[maestro.nombre,maestro.apellido,maestro.status,id],callback);
},
