...
return  db.get().query("Insert into curso values(?,?,?,?,?)",[null,curso.descripcion,curso.inicio,curso.fin,curso.status],callback);
},
...
updatecurso:function(id,curso,callback){
 return  db.get().query("update curso set descripcion=?,inicio=?,fin=? where id=?",[curso.descripcion,curso.inicio,curso.fin,id],callback);
},
